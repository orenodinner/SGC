...
- desktop E2E に `roadmap-stack` の gap / 44px row と `roadmap-overview.bottom === roadmap-panel.top` の実測検証を追加した
- screenshot `artifacts/roadmap-panel-adjacent.png` で `roadmap-overview` height `44px`、`spacing: 0`、stack `gap: 0px` を確認した
- `typecheck / lint / test / build` と targeted desktop E2E を通過した
- `scripts/build.ps1` 初回は artifact runtime の Electron process が `default_app.asar` を掴んで失敗したため、該当 artifact runtime process を終了して再実行した
- `scripts/build.ps1` で Windows portable artifact を再生成し、`artifacts/sgc-portable-win-x64-v0.1.0.zip` のサイズが `159,045,184 bytes` であることを確認した
- `gh` CLI は PATH 上に無く、GitHub release asset upload blocker が継続していることを確認した
...
- `typecheck / lint / test / build` と targeted desktop E2E を通過した
- `scripts/build.ps1` で Windows portable artifact を再生成し、`artifacts/sgc-portable-win-x64-v0.1.0.zip` のサイズが `154,578,158 bytes` であることを確認した
- `gh` CLI は PATH 上に無く、GitHub release asset upload blocker が継続していることを確認した
